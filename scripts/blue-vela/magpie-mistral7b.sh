...
# Instruciton gen tuning
total_prompts=500000
ins_temp=1
ins_topp=0.995
...
echo "[magpie.sh] Output dir: $output_folder" 

echo "[magpie.sh] Start Generating Instructions..."
CUDA_VISIBLE_DEVICES=$device python ${WORKSPACE}/magpie/exp/gen_ins.py \
    --device $device \
...
CUDA_VISIBLE_DEVICES=$device python ${WORKSPACE}/magpie/exp/exp/gen_res.py \
    --device $device \
    --model_path $model_path \
    ...
    --tensor_parallel $tensor_parallel \
    --gpu_memory_utilization $gpu_memory_utilization \
    --input_file $output_folder/Magpie_${model_path##*/}_${total_prompts}_${timestamp}_ins.json \
    --offline \
